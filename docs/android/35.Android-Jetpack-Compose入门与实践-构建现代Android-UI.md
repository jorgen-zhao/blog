```yaml
---
title: Android Jetpack Compose入门与实践 - 构建现代Android UI
date: 2023-11-15 10:00:00
permalink: /pages/android-jetpack-compose/
categories: 
  - Android
tags:
  - Jetpack Compose
...
确保Activity使用`ComponentActivity`：
...
对于Flow，可以使用`collectAsState`来收集状态：
...
`remember`只在组合期间保持状态，而`rememberSaveable`会在配置更改时保持状态：
