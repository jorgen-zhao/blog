...
通过选择`development`, `production`或`none`之中的一个，来设置`mode`参数，你可以启用Webpack内置的优化。默认值为`production`。
...
创建一个`src`目录，并在其中添加一个`index.js`文件：
...
这将生成一个`dist/main.js`文件，包含了我们的应用代码和lodash。

### 使用NPM脚本

为了简化命令，我们可以在`package.json`中添加一个脚本：
...
现在，我们可以通过`npm run build`来运行Webpack。
...
要处理图片和字体等静态资源，我们可以使用`file-loader`或`url-loader`：
...
`webpack-dev-server`提供了一个简单的web服务器，并支持热模块替换(HMR)：
...
编写样式文件`src/index.css`和`src/App.css`：
...
现在，我们可以通过`npm start`启动开发服务器，或者通过`npm run build`构建生产版本。
